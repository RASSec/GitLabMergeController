...
client=MongoClient('mongodb://admin:L3novo@10.62.101.60:27017/')
db=client.PERF
#collection=db.SmbUser
#collection=db.LoyaltyCustomerProfile
#collection=db.rawOrders
...
# condition = {"$nor":[{"userId":{"$regex":".*@lenovo.com"}},{"userId":{"$regex":".*@yopmail.com"}},{"userId":{"$regex":".*@sharklasers.com"}},{"userId":{"$regex":".*@test.com"}},{"userId":{"$regex":".*@leno.com"}},{"userId":{"$regex":"rowanapptest.*"}},{"userId":{"$regex":".*perfasmuser.*"}}]}
